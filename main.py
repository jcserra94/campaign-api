...
async def lifespan(app: FastAPI):
    ...
    with Session(engine) as session:
        if not session.exec(select(Campaign)).first():
            session.add_all([Campaign(name = "Sosa Rise", due_date = datetime.now()),
                             Campaign(name = "Sosa Takeover", due_date = datetime.now())])
            session.commit()
    yield
...
@app.get("/campaigns", response_model=Response[list])
